...
import type { Elements, Vec3 } from '../src/astro/kepler.ts';
...
const expectSmallSolverResidual = (e: number): void => {
  const samples = 64;
...
describe('positionAtTime', () => {
  ...
  it('drifts the orbit when apsidal and nodal precession rates are supplied', () => {
    ...
    expect(
      Math.hypot(drifted.x - still.x, drifted.y - still.y, drifted.z - still.z),
    ).toBeGreaterThan(1);
  });
});
