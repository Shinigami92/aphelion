 ...
 * keypresses away.
 */
export const RATE_PRESETS: ReadonlyArray<RatePreset> = [
...
export class TimeController {
  ...
  get atLimit(): 'min' | 'max' | null {
    if (this._jdUtc <= JD_MIN) {
      return 'min';
    }
    if (this._jdUtc >= JD_MAX) {
      return 'max';
    }
    return null;
  }
  ...
  advance(realDtSeconds: number): void {
    if (this._paused || realDtSeconds === 0) {
      return;
    }
