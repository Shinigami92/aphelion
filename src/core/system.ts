...
import type { BodySpec, BodyType } from '../data/bodies.ts';
...
import { positionAtTime, sampleOrbit, velocityAtTime } from '../astro/kepler.ts';
...
import { PLANET_KEYS, planetOrbitElements, planetPosition } from '../astro/planets.ts';
...
export class SolarSystem {
  ...
  private addMinorPlanets(smallByName: Map<string, SmallBodyData>): void {
    ...
    for (const sb of SMALL_BODIES) {
      ...
      if (this.byKey.has(`moon:${sb.name}`)) {
        continue;
      }
      void smallByName;
