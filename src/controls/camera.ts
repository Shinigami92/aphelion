...
export class CameraController {
  ...
  private updateOrbit(dt: number): void {
    ...
    if (this.keys.orbitLeft) {
      this.targetAzimuth -= rate;
    }
    if (this.keys.orbitRight) {
      this.targetAzimuth += rate;
    }
    if (this.keys.orbitUp) {
      this.targetElevation = Math.min(MAX_ELEVATION, this.targetElevation + rate);
    }
    if (this.keys.orbitDown) {
      this.targetElevation = Math.max(-MAX_ELEVATION, this.targetElevation - rate);
    }
    const zoomRate = Math.exp((this.keys.boost ? 2.4 : 1.1) * dt);
    if (this.keys.zoomIn) {
      this.targetDistance /= zoomRate;
    }
  ...
  private updateFree(dt: number): void {
    ...
    if (this.keys.forward) {
      this.freePosition.addScaledVector(forward, step);
    }
    if (this.keys.back) {
      this.freePosition.addScaledVector(forward, -step);
    }
    if (this.keys.right) {
      this.freePosition.addScaledVector(right, step);
    }
    if (this.keys.left) {
      this.freePosition.addScaledVector(right, -step);
    }
    if (this.keys.up) {
      this.freePosition.addScaledVector(up, step);
    }
    if (this.keys.down) {
      this.freePosition.addScaledVector(up, -step);
    }
