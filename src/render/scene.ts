...
export class SceneView {
  ...
  build(system: SolarSystem): void {
    ...
    this.minorBodies = system.bodies.filter((b) => b !== system.sun && !this.visuals.has(b.key));
  ...
  private createVisual(body: SimBody): BodyVisual {
    ...
    const atmo =
      spec?.atmosphere ?? (body.type === 'moon' ? MOON_ATMOSPHERES[body.name] : undefined);
  ...
  private updateVisual(visual: BodyVisual, scale: ScaleModel, sunSceneRadius: number): void {
    ...
    if (camera) {
      ...
      if (lod !== visual.lod) {
        ...
        if (visual.clouds) {
          visual.clouds.geometry = this.lodGeometries[Math.max(1, lod)]!;
        }
        if (visual.atmosphere) {
          visual.atmosphere.geometry = this.lodGeometries[Math.max(1, lod)]!;
        }
      }
      // Hide anything smaller than a fraction of a pixel; the point cloud and
      // labels still represent it.
      visual.group.visible =
        apparent > 0.35 || body === this.currentFocus || body === this.selected;
    }
  }
  ...
  private updateOrbits(system: SolarSystem, scale: ScaleModel, dt: number): void {
    ...
    if (this.toggles.orbits === 'all') {
      ...
      const host =
        this.currentFocus?.type === 'moon' ? this.currentFocus.parent : this.currentFocus;
  ...
  private updateLabels(system: SolarSystem): void {
    ...
    for (const body of candidates) {
      ...
      if (apparent > this.viewport.y * 0.75) {
        continue;
      }
  ...
  pick(clientX: number, clientY: number, system: SolarSystem, tolerance = 22): SimBody | null {
    ...
    const groups = this.toggles.lagrange ? [system.bodies, this.lagrangeBodies] : [system.bodies];
  ...
  resize(width: number, height: number, camera: PerspectiveCamera): void {
    ...
    this.renderer.setPixelRatio(
      Math.min(window.devicePixelRatio, QUALITY[this.quality].maxPixelRatio),
    );
