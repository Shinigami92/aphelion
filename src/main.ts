...
import { parseView, rateToPreset, UrlWriter } from './core/url-state.ts';
...
function sceneIsChanging(now: number): boolean {
  if (now - lastInteractionAt < ACTIVE_LINGER_MS) {
    return true;
  }
  if (now - camera.lastInputAt < ACTIVE_LINGER_MS) {
    return true;
  }
  if (scale.isTransitioning || camera.isSettling) {
    return true;
  }
  if (!time.paused && Math.abs(time.selectedRate) >= MOVING_RATE) {
    return true;
  }
  ...
  if (!time.paused && scene.ringParticlesActive) {
    return true;
  }
  return false;
}
...
function frame(now: number): void {
  ...
  if (now - lastRenderAt < interval - 0.5) {
    return;
  }
  lastRenderAt = now;
